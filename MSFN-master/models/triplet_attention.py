...
#         self.SelfAttention = SelfAttention(dim = gate_channels, heads = 8, causal = False).cuda()
#     def forward(self, x):
#
#         w =  x.shape[2]
#         h =  x.shape[3]
#         x = rearrange(x, 'b c w h -> b (w h) c')  #Transpose
#         x = self.SelfAttention(x)
#         p = x.shape[1]
#         x = rearrange(x, 'b (p1 p2) c ->b c p1 p2', p1 = w, p2 =h)
#         return x